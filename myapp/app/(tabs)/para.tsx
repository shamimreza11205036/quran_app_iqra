...
export default function Para() {
  ...
  useEffect(() => {
    axios.get('http://ucchashbd.com/api/ayat')
      .then(response => {
        const uniqueSurahs = Array.from(new Set(response.data.map((item: any) => item.para)));
  ...
  const handleSurahSelect = (surah: string) => {
    navigation.navigate('ParaDetails', { surah });
  };

  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>পারা সমুহ</Text>

      <View style={styles.searchContainer}>
        <TextInput
          style={styles.searchInput}
          placeholder="পারা খুঁজুন..."
          value={searchQuery}
      ...
      <View style={styles.buttonContainer}>
        {filteredSurahs.map((surah, index) => (
          ...
          >
            <Text style={styles.buttonText}>পারা {surah}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </ScrollView>
